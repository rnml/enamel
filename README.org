...
enamel -- a functional language with

  Anonymous (i.e. first class) variant/record/recursive types
  ML-style modules, but streamlined
  Type-error slicing
  Lisp-style macros
...
* NOTES FROM IPHONE
** Plan
> Get building under core 108.07
> Add a source syntax: Lex and yacc
> Add a base language:
> ref + pair + int + function
> Explicitly typed at first
> Define syntax and type checker and
> evaluator for tgt language
> Compile from mod+src to tgt
> Add polymorphic
