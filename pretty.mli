...
(* [text "hello"] creates a document containing the string ["hello"].
   This operator simply turns strings into documents. *)
...
(* [nest i doc]: all breaks inside [doc] which are turned into
   ...
   document will followed by [i+j] spaces. *)
...
val pp_to_string : int -> doc -> string
(* [pp_to_string w doc] formats [doc] for line length [w] (> 0) and
   ...
   by a virtual [agrp] - if this is not desired, it should have itself
   a different group at its outermost level to protect itself from
   the [agrp]. *)

val pp_to_file : out_channel -> int -> doc -> unit
(* [pp_to_file oc w doc] pretty prints [doc] for line length [w] into an
   output channel [oc]. The document is considered to be surrounded
   by a virtual [agrp] - if this is not desired, it should have itself
   a different group at its outermost level to protect itself from
   this [agrp]. *)

val print_doc : int -> doc -> unit
(* [print_doc doc] pretty prints [doc] to [stdout] for line length 80.
   The document is considered to be surrounded by a virtual [agrp] - 
   if this is not desired, it should have itself a different group at 
   its outermost level to protect itself from this [agrp]. *)
